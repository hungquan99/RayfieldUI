...
function Hide()
	Debounce = true
	RayfieldLibrary:Notify({Title = "UI Hidden", Content = "The UI has been hidden, you can unhide the interface by tapping F10", Duration = 5})
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if Settings.LoadingTitle ~= "Rayfield Interface Suite" then
		LoadingFrame.Version.Text = "Hung Hub"
	end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						...
						RayfieldLibrary:Notify({Title = "Key System", Content = "The key for Hung Hub has been saved successfully"})
					end
				else
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			UserInputService.InputBegan:Connect(function(input, processed)
				...
				if CheckingForKey then
					if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Enum.KeyCode.F10 then
	...
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				...
				RayfieldLibrary:Notify({Title = "Executor Loaded", Content = "Hung Hub Script Executor UI Has Been Loaded"})
			end
		end)
	end
end
